...
class Body:
    ...
    def get_unnormalization_scales(self, image):
        # # assuming hip is 0.25m wide in normal T pose and belt is always parallel to ground
        # x_dir = np.array([1, 0, 0])
        # hip_dir = np.array(left_hip - right_hip)
        # hip_dir = hip_dir/np.linalg.norm(hip_dir)

        # y_rotation_cos = abs(np.dot(x_dir, hip_dir))
        # hip_projection_length = 0.25*y_rotation_cos

        # hip_dist = abs(left_hip[0] - right_hip[0])

        # frame_width = hip_projection_length/hip_dist
    ...
    def get_hand_transform_wrt_hip(self, side):
        ...
        hip_rotation = np.eye(3) #self.get_hip_rotation()
    ...
    def get_grip(self, side):
        ...
        # print(np.linalg.norm(index_tip - thumb_tip))

        return (np.linalg.norm(index_tip - thumb_tip) / self.hip2neck_dist) > 0.15
